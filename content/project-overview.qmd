...
def rating(threat, barrier):
    if threat == "extent":
        if barrier == "DAM":
            pct = api.barrier_extent(barrier, 'BELA')[1]
            rating = condition(pct)
        elif barrier == "ROAD":
            pct = api.barrier_extent('ROAD, RESOURCE/OTHER', 'BELA')[1] + api.barrier_extent('ROAD, DEMOGRAPHIC', 'BELA')[1]
            rating = condition(pct)
    elif threat == "severity":
        if barrier == "DAM":
            pct = api.barrier_severity(barrier, 'BELA')[2]
            rating = condition(pct)
        elif barrier == "ROAD":
            pct = api.barrier_severity('ROAD, RESOURCE/OTHER', 'BELA')[2] + api.barrier_severity('ROAD, DEMOGRAPHIC', 'BELA')[2]
            rating = condition(pct)
